let handler = m => m 
 ...
     conn.reply(m.chat, `*سارة تحت امرك 🥺🫶🏻💗*`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
 ...
     conn.reply(m.chat, `*قول نعم يسطا 🗿*`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
   ...
   if (/^سارة|ساره$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat,'*سارة بتحبك ياض 🥺🫶🏻💗*',m) //wm, null, [['Menu', '#menu']], m) botones :v

   }

   if (/^بوت خرا|بوت زفت|خرا عليك$/i.test(m.text) ) { //sem prefixo
     conn.reply(m.chat,'  *🗿 دزها يلا انا عمتك*',m) //wm, null, [['Menu','#menu']], m) botones :v

   }
 if (/^بحبك|احبك$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `*امشي ياض غور مش ناقصة تلزيق 🗿*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
 ...
 if (/^اخرسي|اسكتي$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `*حاضر ياقلب سارة 🥺*`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
